...
module.exports = class PromptCompiler {
  ...
  readPrompt(fileName) {
    return fs.readFileSync(path.join(__dirname, this.promptsDirectory, fileName), "utf8");
  }
  ...
  compileRailPrompt(fileName) {
    ...
    if (railProcess.error) {
      ...
      if (railProcess.error) {
        throw new Error("Could not compile rail file. Make sure you have python is installed.");
      }
    }
 ...
 // Read all prompts from the prompts folder and add them to the DefinePlugin.
  ...
  const railPrompts = allPrompts
    ...
    .reduce((acc, fileName) => ({ 
      ...acc, 
      [`process.env.${fileName.split(".")[0]}Prompt`]: this.compileRailPrompt(fileName) 
    }), {});
